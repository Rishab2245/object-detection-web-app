...
import ServerInferenceCamera from '../ServerInferenceCamera';
...
const Yolo = (props: any) => {
  ...
  const [mode, setMode] = useState<'wasm' | 'server'>('wasm');
  const [serverUrl, setServerUrl] = useState<string>(process.env.NEXT_PUBLIC_SERVER_URL || 'http://localhost:5000');
  ...
  }, [modelName, modelResolution, mode]);
